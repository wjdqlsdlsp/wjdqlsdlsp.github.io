...
OLTP는 일살적인 작업을 지원하는데 중점을 두는 반면,
...
OLTP 및 OLAP 시스템은 함께 작동합니다. 그들은 서로를 필요로합니다.

OLTP 데이터는 일반적으로 OLAP 데이터웨어 하우스를 만들기 위해 가져 와서 정리되는 운영 데이터베이스에 저장됩니다. 트랜잭션 데이터가 없으면 분석을 수행 할 수 없습니다.
